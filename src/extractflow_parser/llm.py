...
class LLM:
        ...
        **kwargs: Any,
        ...
        if self.provider == "ollama":
            ...
            try:
                ...
                raise ImportError(
                    "Gemini is not installed. Please install it using pip install 'extractflow_parser[gemini]'."
                )
    ...
    def generate_markdown(self, base64_encoded: str) -> Any:
        ...
        if self.complexity:
            ...
            # Handle response based on provider and compatibility
            if self.provider == "openai" and self.openai_compatible:
    ...
    def _openai(
        ...
        try:
            ...
            if structured:
                if self.base_url and self.openai_compatible:
                    response = self.client["structured"].chat.completions.create(
                        model=self.model_name,
